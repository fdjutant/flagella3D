...
imgs = da.from_npy_stack(intensityFiles[1])
...
# for frame in range(0,1):
    ...
    # threshold image
    img_thresh_med = img_now > 0.3 * np.max(img_now)
    ...
    if frame == 0:   
        ...
        blob = blobs == keep
        print('frame, sizes:', frame, max(sizes))
...
# for frame in range(0,1):
    ...
    fig.suptitle('data: %s\n' %os.path.basename(intensityFiles[0]) +
    ...
    # save to folder
    savingSnapshots = os.path.join(intensityFiles[0], 'snapshots')
...
ax0.plot(xaxis*exp3D_ms,MSD_S2,
