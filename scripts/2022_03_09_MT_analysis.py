...
# for frame in range(0,1):
    ...
    # threshold image
    img_thresh_med = img_now > 0.15 * np.max(img_now)
...
viewer.add_image(imgs, contrast_limits=[0,400],\
                    scale=[0.115,.115,.115],\
...
#%% save movie  
savingPKL = os.path.dirname(whichFiles) + os.path.basename(whichFiles)[-6:-4] 
savingMovie_Time = savingPKL + '-Time.mov'
viewer = napari.Viewer(ndisplay=3)
...
savingMovie_Rotate = savingPKL + '-Rotate.mov'
viewer = napari.Viewer(ndisplay=3)
viewer.add_image(imgs, contrast_limits=[0,300],\
                    scale=[0.115,.115,.115],\
                    multiscale=False,colormap='gray',opacity=1)
