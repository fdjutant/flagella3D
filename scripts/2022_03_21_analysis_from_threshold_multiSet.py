...
thresholdFolder = os.path.join(this_file_dir,
                          ...
                          'Flagella-data', setName, 'Threshold')

thresholdFiles = list(Path(thresholdFolder).glob("*-threshold.npy"))

for whichFiles in range(30,len(thresholdFiles)):
    ...
    # for frame in range(0,1):
        ...
        Un2, Vn2, Wn2 = zip(list(5*n2s[frame])) 
        Un3, Vn3, Wn3 = zip(list(5*n3s[frame]))
        ax3.quiver(X,Y,Z,Un1,Vn1,Wn1,color='C0')
        ax3.quiver(X,Y,Z,Un2,Vn2,Wn2,color='C1')
        ax3.quiver(X,Y,Z,Un3,Vn3,Wn3,color='C2')
