...
# for whichFiles in range(0,5):
    ...
    ax6.plot(xaxis*exp3D_sec, MSD_n1,c='C0',marker="^",mfc='none',
              ms=8,ls='None',alpha=1)   
    ax6.plot(xaxis*exp3D_sec,np.mean([MSD_n2,MSD_n3],axis=0),
              c='k',marker="^",mfc='none',
              ms=8,ls='None',alpha=1)
    ax6.plot(xaxis*exp3D_sec, MSD_n2,c='C1',marker="s",mfc='none',
              ms=8,ls='None',alpha=.3)
    ax6.plot(xaxis*exp3D_sec, MSD_n3,c='C2',marker="s",mfc='none',
              ms=8,ls='None',alpha=.3)
    ax6.plot(xaxis*exp3D_sec, fit_n1_const + fit_n1*xaxis,
             c='C0',alpha=1,label='_nolegend_')
    ax6.plot(xaxis*exp3D_sec, fit_n2n3_const + fit_n2n3*xaxis,
             c='k',alpha=1,label='_nolegend_')
    ax6.plot(xaxis*exp3D_sec, fit_n2_const + fit_n2*xaxis,
             c='C1',alpha=.3,label='_nolegend_')
    ax6.plot(xaxis*exp3D_sec, fit_n3_const + fit_n3*xaxis,
             c='C2',alpha=.3,label='_nolegend_')
    ax6.set_xlabel(r'Lag time [sec]');
    ax6.set_ylabel(r'MSD [$\mu m^2$]')
    # ax0.set_ylim([0, 2]);
    ax6.set_xlim([0, nInterval*exp3D_sec])
    ax6.legend(['$D_\parallel=$ %.3f $\mu$m$^2$/sec' %np.round(fit_n1/(2*exp3D_sec),3),
                # '$D_{\perp}=$ %.3f $\mu$m/sec$^2$' %np.round(fit_n3/(2*exp3D_sec),3) ])
                "$D_{\perp}=$ %.3f, %.3f $\mu$m$^2$/sec" 
                %(np.round(fit_n2/(2*exp3D_sec),3),
                  np.round(fit_n3/(2*exp3D_sec),3)) ])
    
    # roll
    xaxis = np.arange(1,nInterval+1)
    ax7.plot(xaxis*exp3D_sec, MSAD_R,c='C0',marker="s",mfc='none',
              ms=8,ls='None',alpha=1)
    ax7.plot(xaxis*exp3D_sec, fit_R_const + fit_R*xaxis,
             c='C0',alpha=1,label='_nolegend_')
    ax7.set_xlabel(r'Lag time [sec]');
    ax7.set_ylabel(r'MSAD [rad$^2$]')
    # ax0.set_ylim([0, 2]);
    ax7.set_xlim([0, nInterval*exp3D_sec])
    ax7.legend(['$D_{R}=$ %.3f rad$^2$/sec' %np.round(fit_R/(2*exp3D_sec),3)])
    
    # pitch and yaw
    xaxis = np.arange(1,nInterval+1)
    ax8.plot(xaxis*exp3D_sec, MSAD_P,c='C1',marker="s",mfc='none',
              ms=8,ls='None',alpha=.3)
    ax8.plot(xaxis*exp3D_sec, MSAD_Y,c='C2',marker="s",mfc='none',
              ms=8,ls='None',alpha=.3)
    ax8.plot(xaxis*exp3D_sec,np.mean([MSAD_P,MSAD_Y],axis=0),
              c='k',marker="^",mfc='none',
              ms=8,ls='None',alpha=1)
    ax8.plot(xaxis*exp3D_sec, fit_PY_const + fit_PY*xaxis,
             c='k',alpha=1,label='_nolegend_')
    ax8.plot(xaxis*exp3D_sec, fit_P_const + fit_P*xaxis,
             c='C1',alpha=.3,label='_nolegend_')
    ax8.plot(xaxis*exp3D_sec, fit_Y_const + fit_Y*xaxis,
             c='C2',alpha=.3,label='_nolegend_')
    ax8.set_xlabel(r'Lag time [sec]');
    ax8.set_ylabel(r'MSAD [rad$^2$]')
    # ax0.set_ylim([0, 2]);
    ax8.set_xlim([0, nInterval*exp3D_sec])
    ax8.legend(['$D_{P}=$ %.3f rad$^2$/sec' %np.round(fit_P/(2*exp3D_sec),3),
                "$D_{Y}=$ %.3f rad$^2$/sec" %np.round(fit_Y/(2*exp3D_sec),3)])
    
    # co-diffusion: correlation
    xaxis = np.arange(1,nInterval+1)
    ax9.plot(xaxis*exp3D_sec, CO_MSD,c='k',marker="s",mfc='none',
              ms=8,ls='None',alpha=1)
    ax9.plot(xaxis*exp3D_sec, fit_CO_const + fit_CO*xaxis,
             c='k',alpha=1,label='_nolegend_')
    ax9.set_xlabel(r'Lag time [sec]');
    ax9.set_ylabel(r'CO-MSD [$\mu$m x rad]')
    ax9.set_xlim([0, nInterval*exp3D_sec])
    ax9.legend(['$D_{\parallel R}=$ %.3f $\mu$m x rad/sec'
                %np.round(fit_CO/(2*exp3D_sec),3)])
    ...
    summaryFolder = os.path.join(up(up(up(thresholdFiles[whichFiles]))),
                                 'Flagella-data','summary-PKL-5fitPoints','PNG')
