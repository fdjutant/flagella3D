...
A_all_vis = np.concatenate([
                A_70/vis70 / (flagella_length_mean*1e-6),
                A_50/vis50 / (flagella_length_mean_50*1e-6),
                A_40/vis40 / (flagella_length_mean_40*1e-6)
                ])
B_all_vis = np.concatenate([
                B_70/vis70 / (flagella_length_mean*1e-6) / diameter_flagellum,
                B_50/vis50 / (flagella_length_mean_50*1e-6) / diameter_flagellum,
                B_40/vis40 / (flagella_length_mean_40*1e-6) / diameter_flagellum
                ]) 
D_all_vis = np.concatenate([
                D_70/vis70 / (flagella_length_mean*1e-6) / diameter_flagellum**2,
                D_50/vis50 / (flagella_length_mean_50*1e-6) / diameter_flagellum**2,
                D_40/vis40 / (flagella_length_mean_40*1e-6) / diameter_flagellum**2
                ])
...
print("Purcell-std:\n A/vis = %.2E, B = %.2E, D = %.2E"
      %(np.std(Astar_over_vis),
        np.std(Bstar_over_vis),
        np.std(Dstar_over_vis)) )
