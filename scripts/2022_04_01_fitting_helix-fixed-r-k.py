...
import helixFun
from helixFun import f, g, min_fn, model_jacobian
...
for frame in range(frame_start,frame_end):
    ...
    amplitude = 1.
    period = 25; wave_number = 2*np.pi / period
    phase = np.pi/8
    
    def cosine_fn(xx,a): # model for "y" projection (on xz plane)
        return 0.7 * np.cos(0.25*xx + a)

    def sine_fn(xx,a): # model for "z" projection (on xy plane)
        return 0.7 * np.sin(0.25*xx + a)
    
    def cost_fn(a):
        d = ((norm(cosine_fn(xx,a)-yy))**2 + (norm(sine_fn(xx,a)-zz))**2) 
        return d/len(xx)
    ...
    def jacobian_fn(a): #Cost gradient
        ...
        g0 = dy  * np.cos(0.25 * xx + a) + dz  * np.sin(0.25 * xx + a)
        g2 = -dy * np.sin(0.25 * xx + a) + dz  * np.cos(0.25 * xx + a)
        g1 = xx * g2
        
        return np.array([g0.sum(),g1.sum(),g2.sum()])*2 / len(xx)
    
    # init_params = np.array([amplitude, wave_number, phase])
    init_params = np.array([phase])
    lower_bounds = [0, 0, -np.inf, -np.inf]
    upper_bounds = [np.inf, np.inf, np.inf, np.inf]
    
    # results_fit = minimize(cost_fn, init_params, method='powell',
    #                        options={'disp': False})
    # results_fit = minimize(cost_fn, init_params, method='L-BFGS-B', 
    #                 jac=jacobian_fn,
    #                 # bounds = [(0.5,1), (0.2,0.3), (-np.pi,np.pi)],
    #                 # bounds = [(-np.pi,np.pi)],
    #                 options={'disp': False})  
    ...
    ym = f(x,fit_params)                        # mid
    yt = f(x,fit_params)+0.5*fit_params[1]      # top
    yb = f(x,fit_params)-0.5*fit_params[1]      # bot
    zm = g(x,fit_params)                        # mid
    zt = g(x,fit_params)+0.5*fit_params[1]      # top
    zb = g(x,fit_params)-0.5*fit_params[1]      # bot
