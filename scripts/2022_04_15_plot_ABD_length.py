...
#%% sort by length
all_length = np.concatenate([flagella_length_mean_40,
                             flagella_length_mean_50,
                             flagella_length_mean_70])
all_A_per_vis = np.concatenate([A_per_vis_40,
                                A_per_vis_50,
                                A_per_vis_70])
all_B_per_vis = np.concatenate([B_per_vis_40,
                                B_per_vis_50,
                                B_per_vis_70])
all_D_per_vis = np.concatenate([D_per_vis_40,
                                D_per_vis_50,
                                D_per_vis_70])
data_all = np.stack([all_length, all_A_per_vis, all_B_per_vis, all_D_per_vis]).T
data_sorted = data_all[data_all[:,0].argsort()]

#%% Compute A, B, and D
plt.rcParams.update({'font.size': 28})
fig, ax = plt.subplots(dpi=300, figsize=(10,6.2))
plt.plot(data_sorted[:,0], data_sorted[:,1], 'o')
plt.ylabel(r'A [$m$]')
plt.xlabel(r'Length [$\mu m$]')

plt.rcParams.update({'font.size': 28})
fig, ax = plt.subplots(dpi=300, figsize=(10,6.2))
plt.plot(data_sorted[:,0], data_sorted[:,2], 'o')
plt.ylabel(r'B [$m^2$]')
plt.xlabel(r'Length [$\mu m$]')

plt.rcParams.update({'font.size': 28})
fig, ax = plt.subplots(dpi=300, figsize=(10,6.2))
plt.plot(data_sorted[:,0], data_sorted[:,3], 'o')
plt.ylabel(r'D [$m^3$]')
plt.xlabel(r'Length [$\mu m$]')
