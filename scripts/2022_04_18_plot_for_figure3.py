...
pklFiles90 = list(Path(pklFolder).glob("suc90*.pkl"))
...
# sucrose 90 (w/v)
D_trans_90 = np.zeros([len(pklFiles90),3])
D_rot_90 = np.zeros([len(pklFiles90),3])
D_CO_90 = np.zeros(len(pklFiles90))
A_per_vis_90 = np.zeros(len(pklFiles90))
B_per_vis_90 = np.zeros(len(pklFiles90))
D_per_vis_90 = np.zeros(len(pklFiles90))
flagella_length_mean_90 = np.zeros(len(pklFiles90))
flagella_length_std_90 = np.zeros(len(pklFiles90))
cm_90 = []
disp_90 = []
disp_Ang_90 = []
MSD_90 = []
MSAD_90 = []
CO_MSD_90 = []
for j in range(len(pklFiles90)):
    with open(pklFiles90[j], "rb") as f:
          data_loaded = pickle.load(f)
    exp3D_sec = data_loaded["exp3D_sec"]
    pxum = data_loaded["pxum"]
    flagella_length_mean_90[j] = np.mean(data_loaded["flagella_length"])*pxum
    flagella_length_std_90[j] = np.std(data_loaded["flagella_length"])*pxum
    cm_90.append(data_loaded["cm"])
    disp_90.append(data_loaded["disp"])
    disp_Ang_90.append(data_loaded["disp_Ang"])
    D_trans_90[j] = data_loaded["D_trans"]
    D_rot_90[j] = data_loaded["D_rot"]
    D_CO_90[j] = data_loaded["D_co"]
    MSD_90.append(data_loaded["MSD"])
    MSAD_90.append(data_loaded["MSAD"])
    CO_MSD_90.append(data_loaded["CO_MSD"])
    
    D_n1 = D_trans_90[j,0] * 1e-12
    D_n1_psi = D_CO_90[j] * 1e-6
    D_psi = D_rot_90[j,0]
    vis90 = 2.84e-3
    A_per_vis_90[j] = D_psi * kB * T / (D_n1 * D_psi - D_n1_psi**2) / vis90
    B_per_vis_90[j] = -D_n1_psi * kB * T / (D_n1 * D_psi - D_n1_psi**2) / vis90
    D_per_vis_90[j] = D_n1 * kB * T / (D_n1 * D_psi - D_n1_psi**2) / vis90
    
MSD_90 = np.array(MSD_90, dtype=object)
MSAD_90 = np.array(MSAD_90, dtype=object)
CO_MSD_90 = np.array(CO_MSD_90,dtype=object)
cm_90 = np.array(cm_90, dtype=object)
disp_90 = np.array(disp_90, dtype=object)
disp_Ang_90 = np.array(disp_Ang_90, dtype=object)
...
#%% sort by length
length_diff_90 = np.stack([flagella_length_mean_90, flagella_length_std_90, 
                        D_trans_90[:,0], D_trans_90[:,1], D_trans_90[:,2],
                        D_rot_90[:,0], D_rot_90[:,1], D_rot_90[:,2]]).T
length_diff_sorted_90 = length_diff_90[length_diff_90[:,0].argsort()]
...
data_exclusion_90 = length_diff_sorted_90[min_length_um < length_diff_sorted_90[:,0]]
data_exclusion_90 = data_exclusion_90[data_exclusion_90[:,0] < max_length_um]
...
mean_n2 = [np.mean(np.mean([data_exclusion_40[:,3],
           ...
           np.mean(np.mean([data_exclusion_90[:,3],
                            data_exclusion_90[:,4]], axis=0))]
std_n2 = [sem(np.mean([data_exclusion_40[:,3],
          ...
          sem(np.mean([data_exclusion_70[:,3],
                       data_exclusion_70[:,4]], axis=0)),
          sem(np.mean([data_exclusion_90[:,3],
                       data_exclusion_90[:,4]], axis=0))]
xlabel = ["1.77", "1.96", "2.84", "10"] 
...
mean_n2 = [np.mean(np.mean([data_exclusion_40[:,6],
           ...
           np.mean(np.mean([data_exclusion_90[:,6],
                            data_exclusion_90[:,7]], axis=0))]
std_n2 = [sem(np.mean([data_exclusion_40[:,6],
          ...
          sem(np.mean([data_exclusion_70[:,6],
                       data_exclusion_70[:,7]], axis=0)),
          sem(np.mean([data_exclusion_90[:,6],
                       data_exclusion_90[:,7]], axis=0))]
xlabel = ["1.77", "1.96", "2.84", "10"] 
# xlabel = ["16.67%", "18.96%", "27.01%"]
...
sns.stripplot(data=[D_rot_40[:,0], D_rot_50[:,0],
                    D_rot_70[:,0], D_rot_90[:,0]],
              color="purple", alpha=0.5,
...
sns.stripplot(data=[data_exclusion_40[:,6],
                    data_exclusion_50[:,6],
                    data_exclusion_70[:,6],
                    data_exclusion_90[:,6]/6],
              color="C1", alpha=0.5,
              marker="o", size=15, jitter=0.08)
sns.stripplot(data=[data_exclusion_40[:,7],
                    data_exclusion_50[:,7],
                    data_exclusion_70[:,7],
                    data_exclusion_90[:,7]/6],
              color="C2", alpha=0.5,
              marker="o", size=15, jitter=0.08)
ax.set_xticklabels(xlabel)
