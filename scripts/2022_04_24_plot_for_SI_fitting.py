...
ym_cos_ref = cosine_fn(xm,np.array([3,params[which_frame,1], 0]))
ym_sin_ref = sine_fn(xm,np.array([3,params[which_frame,1], 0]))
...
# n1-n2 plot
# =========
rn1_bin_edges = np.linspace(-45, 45, 51)
...
plt.plot(xm, ym_cos, '#E5B65C', lw=3)
...
plt.imshow(h2d, extent=extent_rn1_rm2, origin="lower",
           cmap="seismic", alpha=0.7)
plt.plot(xm, ym_sin, '#E5B65C', lw=3)
plt.xlabel(r'$R \cdot n_1$ [px]')
