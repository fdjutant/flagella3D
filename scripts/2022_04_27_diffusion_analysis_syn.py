...
thresholdFiles = list(Path(thresholdFolder).glob("*.tif"))
intensityFiles = list(Path(intensityFolder).glob("*.tif"))
pklFiles = list(Path(intensityFolder).glob("*.pkl"))
...
# input pkl files
with open(pklFiles[whichFiles], "rb") as f:
      data_loaded = pickle.load(f)

Dpar_input = data_loaded["Dpar"]
Dperp_input = data_loaded["Dperp"]
Dpitch_input = data_loaded["Dpitch"]
Droll_input = data_loaded["Droll"]
Dyaw_input = data_loaded["Dyaw"]
vol_exp_input = data_loaded["vol_exp"]
cm_input_pkl = data_loaded["cm"]  
EuAng_input_pkl = data_loaded["EuAng"]
localAxes_input = data_loaded["localAxes"]
...
fitImage = np.zeros(imgs_thresh.shape,dtype='uint8')
...
# MSD: mean square displacement (input)
n1s_input = localAxes_input[:,0]
n2s_input = localAxes_input[:,1]
n3s_input = localAxes_input[:,2]
MSD_n1_in, MSD_n2_in, MSD_n3_in, CO_MSD_in = msd.trans_MSD_Namba(nt,
                                          cm_input_pkl,
                                          EuAng_input_pkl[:,1],
                                          n1s_input, n2s_input, n3s_input,
                                          exp3D_sec, nInterval)
MSAD_P_in = msd.regMSD_Namba(nt, EuAng_input_pkl[:,0], exp3D_sec, nInterval)
MSAD_R_in = msd.regMSD_Namba(nt, EuAng_input_pkl[:,1], exp3D_sec, nInterval)
MSAD_Y_in = msd.regMSD_Namba(nt, EuAng_input_pkl[:,2], exp3D_sec, nInterval)
...
fit_n1_in, fit_n1_c_in  = optimize.curve_fit(MSDfit, xtime, MSD_n1_in[0:Nfit])[0]
fit_n2_in, fit_n2_c_in  = optimize.curve_fit(MSDfit, xtime, MSD_n2_in[0:Nfit])[0]
fit_n3_in, fit_n3_c_in  = optimize.curve_fit(MSDfit, xtime, MSD_n3_in[0:Nfit])[0]
fit_R_in, fit_R_c_in   = optimize.curve_fit(MSDfit, xtime, MSAD_R_in[0:Nfit])[0]
fit_P_in, fit_P_c_in  = optimize.curve_fit(MSDfit, xtime, MSAD_P_in[0:Nfit])[0]
fit_Y_in, fit_Y_c_in  = optimize.curve_fit(MSDfit, xtime, MSAD_Y_in[0:Nfit])[0]
...
#%% compare CM movement
cm_origin = cm - data_loaded["origin"]
xtrack = np.arange(0,len(EuAng_computed[:,0]))[:-15]

cm_origin = cm_origin[:-15]
cm_input = cm_input_pkl[:-15]
...
ds_r = 2 # down sampling for plotting

ax0.plot(xtrack[::ds_r]*exp3D_sec, cm_origin[:,1][::ds_r],
         alpha=1, c='g', marker='o',
         markersize=5, mfc='None')   
ax0.plot(xtrack[::ds_r]*exp3D_sec, cm_input[:,1][::ds_r],
         alpha=0.5, c='k', marker='o',
         markersize=5, mfc='None')   
ax0.set_ylabel(r'$x$ [$\mu$m]')
# ax0.set_ylim([-20,5])
ax0.set_xlim([-0.2, 10.2])

ax1.plot(xtrack[::ds_r]*exp3D_sec, cm_origin[:,0][::ds_r],
         alpha=1, c='g', marker='o',
         markersize=5, mfc='None')    
ax1.plot(xtrack[::ds_r]*exp3D_sec, cm_input[:,0][::ds_r],
         alpha=0.5, c='k', marker='o',
         markersize=5, mfc='None')  
...
ax2.plot(xtrack[::ds_r]*exp3D_sec, cm_origin[:,2][::ds_r],
         alpha=1, c='g', marker='o',
         markersize=5, mfc='None')    
ax2.plot(xtrack[::ds_r]*exp3D_sec, cm_input[:,2][::ds_r],
         alpha=0.5, c='k', marker='o',
         markersize=5, mfc='None')  
...
ax2.figure.savefig(pdfFolder + '/tracking-CM-syn.pdf') 


#%% compare orientation tracking
xtrack = np.arange(0,len(EuAng_computed[:,0]))[:-15]
EuAng_input = EuAng_input_pkl[:-15]
EuAng = EuAng_computed[:-15]
...
ax0.plot(xtrack[::ds_r]*exp3D_sec, EuAng[:,1][::ds_r],
         alpha=1, c='g', marker='o',
         markersize=5, mfc='None')   
ax0.plot(xtrack[::ds_r]*exp3D_sec, EuAng_input[:,1][::ds_r],
         alpha=0.5, c='k', marker='o',
         markersize=5, mfc='None')   
ax0.set_ylabel(r'$\psi$ [rad]')
ax0.set_xlim([-0.2, 10.2])

ax1.plot(xtrack[::ds_r]*exp3D_sec, EuAng[:,0][::ds_r],
         alpha=1, c='g', marker='o',
         markersize=5, mfc='None')    
ax1.plot(xtrack[::ds_r]*exp3D_sec, EuAng_input[:,0][::ds_r],
         alpha=0.5, c='k', marker='o',
         markersize=5, mfc='None')  
...
ax2.plot(xtrack[::ds_r]*exp3D_sec, EuAng[:,2][::ds_r],
         alpha=1, c='g', marker='o',
         markersize=5, mfc='None')    
ax2.plot(xtrack[::ds_r]*exp3D_sec, EuAng_input[:,2][::ds_r],
         alpha=0.5, c='k', marker='o',
         markersize=5, mfc='None')  
# ax2.set_ylim([-1,0.3])
ax2.set_xlim([-0.2, 10.2])
ax2.set_ylabel(r'$\beta$ [rad]')
ax2.set_xlabel(r'Time [sec]')
ax2.figure.savefig(pdfFolder + '/tracking-EuAng-syn.pdf') 

#%% Compare MSD and MSAD
nInterval = 50
...
ax0.plot(xaxis*exp3D_sec, MSD_n1,
         alpha=1, c='g', marker='o',
         markersize=8, mfc='None', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSD_n1_in,
         alpha=0.5, c='k', marker='o',
         markersize=8, mfc='k', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSD_n2,
         alpha=1, c='g', marker='s',
         markersize=8, mfc='None', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSD_n2_in,
         alpha=0.5, c='k', marker='s',
         markersize=8, mfc='k', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSD_n3,
         alpha=1, c='g', marker='^',
         markersize=8, mfc='None', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSD_n3_in,
         alpha=0.5, c='k', marker='^',
         markersize=8, mfc='k', linestyle='None')
ax0.plot(xaxis*exp3D_sec, fit_n1_const + fit_n1*xaxis,
          c='g',alpha=1,label='_nolegend_')
...
ax0.figure.savefig(pdfFolder + '/MSD-syn.pdf') 
...
ax0.plot(xaxis*exp3D_sec, MSAD_R,
         alpha=1, c='g', marker='o',
         markersize=8, mfc='None', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSAD_R_in,
         alpha=0.5, c='k', marker='o',
         markersize=8, mfc='k', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSAD_P,
         alpha=1, c='g', marker='s',
         markersize=8, mfc='None', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSAD_P_in,
         alpha=0.5, c='k', marker='s',
         markersize=8, mfc='k', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSAD_Y,
         alpha=1, c='g', marker='^',
         markersize=8, mfc='None', linestyle='None')
ax0.plot(xaxis*exp3D_sec, MSAD_Y_in,
         alpha=0.5, c='k', marker='^',
         markersize=8, mfc='k', linestyle='None')
...
ax0.figure.savefig(pdfFolder + '/MSAD-roll-syn.pdf') 
...
ax0.set_xlabel(r'Lag time [sec]')
ax0.set_ylabel(r'MSAD [$rad^2$]')
ax0.figure.savefig(pdfFolder + '/MSAD-PY-syn.pdf') 
