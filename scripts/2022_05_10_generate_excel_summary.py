...
prop_eff = np.zeros(total_file_number)
...
for index_files in range(total_file_number):
    ...
    prop_eff[index_files] = 100 * B_per_vis[index_files]**2 /\
                            (4* A_per_vis[index_files] * D_per_vis[index_files])
...
df = pd.DataFrame(summary_data,
                  columns = ['data name', 'number of frames',
                             ...
                             'A/eta [m]', 'B/eta [m^2]',
                             'D/eta [m^3]', 'efficiency [%]'
                             ])
print(df)
excel_path = os.path.join(result_dir_csv, 'all-flagella-data.xlsx')
