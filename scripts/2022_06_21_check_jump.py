...
intensityFolder = os.path.join(this_file_dir, 'TIF-files')
pklFolder = os.path.join(this_file_dir, 'PKL-files')

thresholdFiles = list(Path(thresholdFolder).glob("*-LabKit-*.tif"))
intensityFiles = list(Path(intensityFolder).glob("*.tif"))
pklFiles = list(Path(pklFolder).glob("*.pkl"))

for whichFiles in range(88):
    
    # chose which file to analyze
    # whichFiles = 82; stframe = 0
    imgs_thresh = tifffile.imread(thresholdFiles[whichFiles])#[stframe:stframe+250]
    imgs = tifffile.imread(intensityFiles[whichFiles])#[stframe:stframe+250]
    # print(intensityFiles[whichFiles].name, len(imgs))
    # print(thresholdFiles[whichFiles].name, len(imgs_thresh))
    
    # open pickles
    pickle_files = list(Path(pklFolder).glob('*.pkl'))
    with open(pickle_files[whichFiles], "rb") as f:
        individual_data = pickle.load(f)
    # print(whichFiles, individual_data["data_name"]+'.pkl')
    # print(len(individual_data["flagella_length"]),
    #       np.mean(individual_data["flagella_length"]),
    #       np.std(individual_data["flagella_length"]))
    ...
    print(whichFiles, thresholdFiles[whichFiles].name)
    print(len(individual_data["flagella_length"]),
          np.mean(individual_data["flagella_length"]),
          np.std(individual_data["flagella_length"]), 'pickle-file')
    print(len(flagella_len),
          np.mean(flagella_len),
          np.std(flagella_len), 'current-TIF')

#%% tracing the cut from the length
num_frames = 250
for i in range(len(flagella_len)-num_frames+1):
    print(i,
          np.mean(flagella_len[i:i+num_frames]),
          np.std(flagella_len[i:i+num_frames]) )
